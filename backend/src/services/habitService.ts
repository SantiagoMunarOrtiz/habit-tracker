...
export const checkInHabit = async (habitId: string, date: string, status: string, note?: string) => {
...
export const calculateWeeklyProgress = async (habitId: string, startDateStr: string, endDateStr: string) => {
  const habit = await prisma.habit.findUnique({
    where: { id: habitId },
    include: {
      logs: {
  ...
  let mandatoryDaysOfWeek: number[] = [];
  ...
  let mandatoryDays: string[] = [];
  let optionalDays: string[] = [];
  let optionalCompleted: string[] = [];
  let missedMandatory: string[] = [];
  let completedMandatory: string[] = [];
  ...
  let replacedMandatory: { missed: string, replacedBy: string }[] = [];
